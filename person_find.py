...
def detect(cfg,
           ...
           video='data/video/qh1.mp4', # input video
           ...
           save_txt=False,
           save_images=False):
    """需要注意的是，行人检测与行人重识别如何更好的进行结合。
    reid使得行人检测可以存在多一些的误检，要求recall比较高，因为一些背景可以通过reid
    进行相似度比较从而排除掉。但是如过置信度阈值设的太低，会有很多的proposal要进入reid模型，降低模型的速度。
    因此，需要进行合理的设置。
    dist_thres 阈值可能有时候需要根据自己的具体情况进行一些调整。 """
    opt.webcam = True
    ...
    if opt.webcam:
        save_images = False
        dataloader = LoadWebcam(video, img_size=img_size, half=opt.half)
    else:
        ...
        if det is not None and len(det) > 0:
            ...
            if gallery_img:
                ...
                if distmat[index] < dist_thres:
                    ...
                    plot_one_box(gallery_loc[index], im0, label='got', color=colors[int(cls)])
